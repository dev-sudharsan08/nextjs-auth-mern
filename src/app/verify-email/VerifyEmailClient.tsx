...
const VerifyEmailClient: React.FC<Props> = ({ token }) => {
  ...
  const [{ isError, message }, setIsError] = useState({ isError: false, message: '' });
  ...
  useEffect(() => {
    if (token) {
      ...
      setHasToken(false);
      setIsError({ isError: true, message: 'Verification token is missing. Please check the link from your email.' });
    }
  ...
  async function handleVerifyEmail(tokenValue: string) {
    ...
    setLoader(true);
    try {
      const response = await axios.post('/api/users/verifyemail', { token: tokenValue });
      ...
      if (axios.isAxiosError(error) && error.response) {
        const errorMessage = error.response.data?.error || 'Verification failed. The token may be invalid or expired.';
    ...
    if (loader && hasToken) {
      return (
        <div className='text-center'>
          ...
          <p className='text-slate-400'>Please wait while we validate your account.</p>
        </div>
      );
    }
    ...
    if (isVerified) {
      return (
        <div className='text-center'>
          ...
          <h1 className='text-2xl sm:text-3xl font-bold text-white mb-4'>Email Verified Successfully! 🎉</h1>
          <p className='text-slate-400 mb-6'>{`Your email is now verified. You will be redirected to the ${authenticated ? 'dashboard' : 'login'} page shortly.`}</p>
    ...
    if (isError) {
      return (
        <div className='text-center'>
          ...
          <p className='text-slate-300 mb-6'>{message || 'The verification link is invalid, expired, or something went wrong.'}</p>

          <div className='space-y-3'>
            <Link href='/login' className='inline-flex items-center justify-center w-full px-8 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-semibold rounded-xl shadow-lg transition-all duration-300 hover:scale-[1.03] active:scale-100'>
              {`Go to ${authenticated ? 'Dashboard' : 'Login'} Page`}
            </Link>
            <p className='text-sm text-slate-500 text-white'>If you believe this is an error, please try logging in or contact support.</p>
          </div>
        </div>
      );
    }
